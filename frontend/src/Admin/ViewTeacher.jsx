...
export default function ViewTeacher() {
    ...
    const getSingleData = async () => {
        try {
            const { data } = await axios.get(`https://school-manage-ment.onrender.com/api/teachers/getSingle/${id}`);
                        ...
                        <div className="w-full h-[55vh] lg:w-1/4 bg-white rounded-xl shadow p-4 space-y-4">
                            <div className="flex items-center space-x-4">
                                {teacherDetails?.userId?.profileImage ? (
                                    <img
                                        src={`https://school-manage-ment.onrender.com/${teacherDetails.userId.profileImage}`}
                                        alt="avatar"
